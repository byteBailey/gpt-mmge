...
def setup_logger(args):
    ...
    log_path = os.path.join(
        args.output_dir,
        f'{args.dataset}_{args.model_name}_{args.llm_model_name}_seed{args.seed}.log'
    )
...
def main(args):
    ...
    # Step 5. Training
    logger.info(f'training set batches: {len(train_loader)}')
    ...
    for epoch in range(args.num_epochs):
        ...
        logger.info(f"Epoch: {epoch}|{args.num_epochs}: Train Loss (Epoch Mean): {epoch_loss / len(train_loader):.6f}")
        ...
        with torch.no_grad():
            ...
            logger.info(f"Epoch: {epoch}|{args.num_epochs}: Val Loss: {val_loss:.6f}")
        ...
        logger.info(f'Epoch {epoch} Val Loss {val_loss:.6f} Best Val Loss {best_val_loss:.6f} Best Epoch {best_epoch}')
